...
PATCHED VERSION - Fixed self-modification filename check
"""
...
# Constants - PATCHED: Now uses the actual script filename
AGENT_FILENAME = Path(__file__).name  # This will be the actual script name
...
class ModernChatUI(tk.Tk):
    ...
    def start_agent(self):
        ...
        # Modified "zero-shot" system prompt
        system_prompt = "You are a helpful AI assistant. Use the provided tools to achieve the user's goal. Prioritize using the tools to solve the problem."
    ...
    def update_ui(self):
        ...
        # Modified to update at ~60 FPS (1000ms / 60 ≈ 16ms)
...
if __name__ == "__main__":
    # PATCHED: Now checks if the script exists with its actual filename
    if not Path(__file__).exists():
        logger.error(f"FATAL: Cannot find script file '{Path(__file__).name}'")
        sys.exit(1)
        
    if not ASYNC_MODE:
        logger.warning("`aiohttp` is not installed. The agent will run in synchronous mode, which may be slower.")
    
    logger.info(f"DGM-Agent running as: {AGENT_FILENAME}")
